...
export async function deploymentSelectionFromOptions(ctx, options, extraCloudFlags = []) {
  if (options.envFile) {
  ...
  return {
    kind: options.prod === true || options.implicitProd === true ? "ownProd" : "ownDev",
    partitionId
  };
}
export async function selfHostedCredentialsFromEnv(ctx, options, cloudFlags) {
  if (options.envFile) {
    dotenv.config({ path: options.envFile });
  } else {
    dotenv.config({ path: ENV_VAR_FILE_PATH });
...
export async function fetchDeploymentCredentialsProvisionProd(ctx, deploymentSelection, { ensureLocalRunning } = { ensureLocalRunning: true }) {
