...
export async function deploymentCredentialsOrConfigure(ctx, chosenConfiguration, cmdOptions, partitionId) {
  ...
  if (cmdOptions.deploymentSelection.kind !== "ownDev" && cmdOptions.deploymentSelection.kind !== "ownProd" && cmdOptions.deploymentSelection.kind !== "deployKey") {
  ...
  const deploymentOptions = cmdOptions.deploymentSelection.kind === "ownProd" ? { kind: "prod" } : devDeployment === "local" ? { kind: "local", ...cmdOptions.localOptions } : { kind: "dev" };
  const {
    deploymentName,
    deploymentUrl: url,
    adminKey
