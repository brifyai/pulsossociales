...
const dashboard = new import_extra_typings.Command("dashboard").description("Open the dashboard in the browser").allowExcessArguments(false).option(
  ...
  const deploymentSelection = await (0, import_api.deploymentSelectionFromOptions)(
    ctx,
    options
  );
  ...
  if (deploymentName === void 0) {
    ...
    if (deploymentSelection.kind === "urlWithAdminKey") {
      printedMessage = `Self-hosted deployment configured. \`${import_chalk.default.bold("npx convex dashboard")}\` is not supported for self-hosted deployments. See self-hosting instructions for how to self-host the dashboard.`;
    } else {
