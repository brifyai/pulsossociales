...
WITH agent_subset AS (
    SELECT id FROM agents WHERE region_id = 'metropolitana' LIMIT 5
),
event_subset AS (
...
WITH agent_subset AS (
    SELECT id FROM agents WHERE region_id = 'valparaiso' LIMIT 5
),
event_subset AS (
...
WITH agent_subset AS (
    SELECT id FROM agents WHERE region_id = 'biobio' LIMIT 5
),
event_subset AS (
...
WITH agent_subset AS (
    SELECT id FROM agents WHERE region_id = 'araucania' LIMIT 5
),
event_subset AS (
...
WITH agent_subset AS (
    SELECT id, region_id FROM agents LIMIT 20
),
national_events AS (
...
ON CONFLICT (agent_id, event_id) DO NOTHING; -- Skip if already exists
